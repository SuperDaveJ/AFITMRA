...
var nLessons_2 = 10;		//module 2 lessons
...
function gotoPage(direction, pgURL) {
	...
	if (contentFrame.isNextDisabled == false) {
	  if (direction == "f" ) {
...
function getLesson() {
	...
	if ( strTemp.indexOf("menu") > 0 ) return 0;
		...
		if (strTemp.length == 6)
			return parseInt(strTemp.substr(1,2));
		else
...
function startCourse() {
	...
	if (inLMS == true) {
		...
		if ( Get_Cookie("userData_mra") ) {
			getSuspendData();
		} else {
			Set_Cookie("userData_mra", initialStatus, 60, "/", "", "");
			strAllLessonStatus = initialStatus;
		}
		if ( Get_Cookie("userBookmark_mra") ) {
			bookmark = Get_Cookie("userBookmark_mra");
...
function exitCourse() {
	if ( exitPageStatus != true ) {
		...
		if ( inLMS == true ) {
			...
			Set_Cookie("userBookmark_mra", contentFrame.location.href, 60, "/", "", "");
		}
		window.close();
	}
}
...
function updateSuspendData() {
	...
	if (inLMS == true) {
		...
		Set_Cookie("userData_mra", strTemp, 60, "/", "", "");
	}
}
...
function getSuspendData() {
	if (inLMS == true) {
		...
		strTemp = Get_Cookie("userData_mra");
	}
	if ( (strTemp != "") && (typeof(strTemp) != "undefined") ) {
		arrTemp = new Array();
